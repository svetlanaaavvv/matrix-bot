...
def calculate_matrix(birth_date: str):
    ...
    try:
        ...
        # B — месяц рождения
        B = to_archan(month)
        # C — сумма цифр года рождения
        C = to_archan(sum(int(d) for d in str(year)))
        # D = A + B + C
        D = to_archan(A + B + C)

        # === ЛИЧНЫЙ (ДИАГОНАЛЬНЫЙ) КВАДРАТ ===
        # E — центр личного квадрата
        E = to_archan(A + B + C + D)

        # Точки личного квадрата
        J = to_archan(A + E)
        K = to_archan(B + E)
        L = to_archan(C + E)
        M = to_archan(D + E)
        ...
        # L1 — центр всей матрицы (ЗОНА КОМФОРТА)
        L1 = to_archan(E + L2)

        # Точки родового квадрата (первая группа)
        F2 = to_archan(F + L2)
        G2 = to_archan(G + L2)
        H2 = to_archan(H + L2)
        I2 = to_archan(I + L2)
        ...
        # === ОТВЕТ БОТА ===
        # Для совместимости с текущим ботом используем ключи:
        # character, karma, money, love, comfort
        return {
...
if __name__ == "__main__":
    ...
    if result:
        ...
        print()
        print(f"  📊 Центр личного квадрата (E): {result['E']}")
        print(f"  📊 Центр родового квадрата (L2): {result['L2']}")
        print(f"  🎯 Личное предназначение: {result['personal_purpose']}")
        print(f"  🎯 Социальное предназначение: {result['social_purpose']}")
        print(f"  🎯 Духовное предназначение: {result['spiritual_purpose']}")
    ...
    if result2:
        ...
        # Проверка, что зона = 10
        if result2['comfort'] == 10:
            print("  ✅ Зона комфорта = 10 — ВЕРНО!")
        else:
            print(f"  ❌ Зона комфорта = {result2['comfort']} — НЕВЕРНО! Должно быть 10")
